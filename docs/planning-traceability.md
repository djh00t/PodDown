...
The authoritative merged baseline is `main` at `7ce7d6f`. The current
reconciliation overlay contains uncommitted P1–P5 implementation and test
evidence; those local results are labelled separately from merged, hosted,
authenticated, live-provider, and release evidence.
...
| 3 | Episode job API | 004 | M3 lifecycle, provider-free API, command receipts, status, source-only snapshot boundary, PostgreSQL repository composition, and Temporal dispatch are locally covered: [episode API BDD](../tests/features/episode_api.feature), [Temporal API BDD](../tests/features/api_temporal_integration.feature), [API integration tests](../tests/integration/test_episode_api.py), [Temporal wire tests](../tests/integration/test_temporal_production_wire.py), and [runtime verification](verification/runtime-composition.md); hosted API/Temporal restart and authenticated UAT remain pending |
...
| 12 | Transcription/fidelity QA | 001/003 | M0/M2 locally verified for normalized transcript QA, audio-bound checksum/empty-text rejection, deterministic and host-local modes, explicit OpenAI ASR boundaries, failure mapping, replay, immutable quality evidence, and provider usage/cost contracts: [provider BDD](../tests/features/provider_temporal_render.feature), [OpenAI contract tests](../tests/contract/providers/test_openai_transcription.py), [activity tests](../tests/unit/audio/test_activities.py), [provider evidence verification](verification/provider-evidence.md), and [fidelity verification](verification/transcription-fidelity-qa.md); credentialed provider-ASR 100% critical-token evidence and billing reconciliation remain pending |
...
| 16 | Package/provenance | 001/003/004 | M0–M5 locally verified for content-addressed immutable artifacts, deterministic nine-file generation, final checksum and critical-token binding, replay, conflict preservation, tenant/project-scoped filesystem and S3 reference ports, conservative orphan discovery/GC activity, and compensated publication references: [package BDD](../tests/features/package.feature), [package-generation BDD](../tests/features/package_generation.feature), [object-maintenance BDD](../tests/features/object_maintenance.feature), [package unit tests](../tests/unit/test_packages.py), [S3/object integration tests](../tests/integration/test_minio_object_storage.py), [immutable package verification](verification/immutable-package.md), [production data-plane verification](verification/production-data-plane.md), and [publishing verification](verification/publishing.md); hosted storage, migration, and recovery drills remain pending |
| 17 | Publishing adapters | 007 | M5 locally verified for QA/checksum-gated immutable publication, filesystem/S3/RSS adapters, explicit HTTP Transistor transport behind opt-in, durable receipt repositories, approval/disclosure/provenance, retry/compensation, Temporal publication activity, and scoped idempotency: [publishing BDD](../tests/features/publishing.feature), [publishing service tests](../tests/unit/test_publishing_service.py), [adapter tests](../tests/unit/test_publishing_adapters.py), [Transistor contract tests](../tests/contract/providers/test_transistor_publishing.py), [resource/publication verification](verification/resource-links.md), and [publishing verification](verification/publishing.md); live S3/Transistor mutation and cross-process receipt/recovery evidence remain pending |
| 19 | MCP server | 006 | M5 locally verified for tenant-authenticated preview/render/publish/status/episode tools, MCP negotiation and stdio plumbing, HTTP gateway, scoped approvals, resource-link redaction, and API-mode auth boundaries: [agent integration BDD](../tests/features/agent_integrations.feature), [HTTP gateway BDD](../tests/features/http_agent_gateway.feature), [resource-link BDD](../tests/features/resource_links.feature), [MCP contract tests](../tests/contract/test_agent_mcp_contract.py), [gateway integration tests](../tests/integration/test_agent_mcp.py), and [agent/resource verification](verification/agent-integrations.md); deployed gateway, MinIO-backed retrieval, and authenticated UAT remain pending |
...
| 22 | Multitenancy | 004 | M3 tenant-scoped lifecycle and PostgreSQL repositories, RLS transaction context, object references, HTTP scope enforcement, and verified OIDC principal boundaries are locally covered: [tenant/RLS BDD](../tests/features/tenant_context.feature), [authenticated API BDD](../tests/features/authenticated_api.feature), [PostgreSQL tenant integration tests](../tests/integration/test_postgres_tenant_boundary.py), [OIDC integration tests](../tests/integration/test_authenticated_oidc_api.py), and [authenticated API verification](verification/authenticated-api.md); real PostgreSQL RLS deployment and customer UAT remain pending |
| 23 | Usage/cost metering | 004 | M3 append-only SQLite and PostgreSQL usage/evidence ledgers, exact Decimal serialization, provider-request replay/conflict protection, and outbox linkage are locally covered: [usage BDD](../tests/features/usage_ledger.feature), [provider evidence integration tests](../tests/integration/test_provider_evidence_persistence.py), [usage repository tests](../tests/unit/test_usage_repository.py), and [usage verification](verification/usage-ledger.md); live provider reconciliation and invoice evidence remain pending |
...
- Production-Closure P1–P5 code paths now exist in the reconciliation overlay:
  live-provider adapter boundaries, PostgreSQL/RLS/ledger/outbox ports,
  API-to-Temporal composition, OIDC/resource-link/MCP boundaries, durable
  publication adapters, operations/retention/capacity contracts, SBOM and
  security scans, and the host-local reference verifier. They remain
  uncommitted local evidence until separately delivered through the normal
  review path; live providers, hosted services, external publication, and
  authenticated UAT remain explicit deployment-boundary work.
