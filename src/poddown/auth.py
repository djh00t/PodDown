...
class AuthSettings:
    ...
    def __post_init__(self) -> None:
        if self.mode not in {"api", "local"}:
            raise ValueError("auth mode must be api or local")
        if self.mode == "api":
            ...
            raise ValueError("local mode cannot carry OIDC metadata")

    @classmethod
    def from_environment(cls) -> AuthSettings:
        """Load runtime settings; API mode is the default and fails closed."""
