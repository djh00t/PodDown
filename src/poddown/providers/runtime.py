...
class LiveProviderRuntime:
    ...
    reasoning_selection: ReasoningModelSelection
    ...
    def from_environment(
        ...
        *,
        ...
        reasoning_selection: ReasoningModelSelection | None = None,
    ) -> LiveProviderRuntime:
        ...
        selection = reasoning_selection or _selection_from_environment(environment)
        configured_reasoning_model = environment.get(
            "PODDOWN_OPENAI_REASONING_MODEL", ""
        ).strip()
        if configured_reasoning_model and configured_reasoning_model != selection.model:
            raise ValueError("configured reasoning model does not match selection")
...
def _selection_from_environment(
    environment: Mapping[str, str],
) -> ReasoningModelSelection:
    raw = _required_environment(environment, "PODDOWN_OPENAI_REASONING_SELECTION_JSON")
    try:
        value = json.loads(raw)
        if not isinstance(value, Mapping):
            raise ValueError("selection record must be an object")
        return parse_reasoning_model_selection_record(value)
    except (json.JSONDecodeError, TypeError, ValueError):
        raise ValueError("PODDOWN_OPENAI_REASONING_SELECTION_JSON is invalid") from None
