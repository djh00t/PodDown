...
def package_sha256_for(package: EpisodePackage, artifact_store: ArtifactStore) -> str:
    ...
    return package_sha256_for_artifacts(package, artifacts)


def package_sha256_for_artifacts(
    package: EpisodePackage, artifacts: Mapping[str, bytes]
) -> str:
    """Return the package-bytes digest from already verified artifact bytes."""
    if not isinstance(package, EpisodePackage):
        raise PackageError("package must be an EpisodePackage")
    if not isinstance(artifacts, Mapping):
        raise PackageIntegrityError("package artifacts are malformed")
    _validate_manifest_contract(package)
    digest = sha256(b"poddown-package-bytes-v1\0")
    for reference in package.files:
        data = artifacts.get(reference.name)
        if not isinstance(data, bytes):
            raise PackageIntegrityError("package artifact bytes are malformed")
        if (
            ...
            raise PackageIntegrityError("package artifact checksum does not match")
