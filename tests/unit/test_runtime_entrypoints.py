...
def test_container_contract_includes_the_postgresql_runtime_library() -> None:
...
def test_worker_outbox_relay_composes_durable_ports_when_enabled(
    ...
    assert callable(activities[0])
    assert initialized == [sentinel_factory]
    assert captured["kwargs"] == {"subject_prefix": "poddown.outbox.v1"}
...
def test_worker_readiness_marker_tracks_started_worker_and_cleanup(
    ...
    class FakeWorker:
        ...
        def __init__(self, *_args, **kwargs) -> None:
            ...
            type(self).activity_executor = kwargs.get("activity_executor")
    ...
    assert isinstance(FakeWorker.activity_executor, ThreadPoolExecutor)
