...
def test_publication_receipt_migration_quotes_postgresql_reserved_authorization() -> (
    None
):
    """The receipt schema must remain valid on a fresh PostgreSQL database."""
    publication_migration = next(
        migration for migration in MIGRATIONS if migration.version == 9
    )

    assert '"authorization" JSONB NOT NULL' in publication_migration.sql
