...
def _environment() -> dict[str, str]:
    ...
    return {
        ...
        "PODDOWN_OPENAI_REASONING_SELECTION_JSON": json.dumps(
            {
...
@when("the evaluated reasoning selection is removed")
def remove_reasoning_selection(context) -> None:
    environment = dict(context.values["environment"])
    environment.pop("PODDOWN_OPENAI_REASONING_SELECTION_JSON")
    with pytest.raises(ValueError) as error:
        LiveProviderRuntime.from_environment(environment)
    context.values["error"] = error.value
...
@then("the runtime uses the evaluated reasoning model")
def runtime_uses_selected_reasoning_model(context) -> None:
    assert context.values["runtime"].reasoning_selection.model == "gpt-4.1-mini"
...
def runtime_fails_closed(context) -> None:
    assert (
        ...
        or "reasoning" in str(context.values["error"]).casefold()
    )
