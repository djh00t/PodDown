...
`PYDANTIC_DISABLE_PLUGINS=1 PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/pytest -q -p pytest_bdd.plugin tests/bdd/test_production_closure_evidence_truth.py` failed as expected with 14 `StepDefinitionNotFoundError` failures before the C07 bindings existed.
...
## Corrected focused BDD output

```text
$ PYDANTIC_DISABLE_PLUGINS=1 PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/pytest -q -p pytest_bdd.plugin tests/bdd/test_production_closure_evidence_truth.py
..............                                                           [100%]
14 passed in 0.02s
```

This command was local-only and did not select `live_provider` tests. No broad test suite was run.
